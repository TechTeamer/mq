...
class GatheringServer {
    ...
    initialize() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const serverQueue = yield channel.assertQueue('', this._assertQueueOptions);
                const serverQueueName = serverQueue.queue;
                yield channel.prefetch(this._prefetchCount);
                yield channel.bindQueue(serverQueueName, this.name, '');
                yield channel.consume(serverQueueName, (msg) => {
                    this._handleGatheringAnnouncement(channel, msg).catch(() => {
                        // this should not throw
                    });
                });
                yield channel.assertQueue(this.statusQueue, {
                    exclusive: false,
                    autoDelete: true
                });
                yield channel.consume(this.statusQueue, () => {
                    // noop
                }, { noAck: true });
            }
            catch (err) {
     ...
     * @private
     */
    _handleGatheringAnnouncement(channel, msg) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let answer;
            try {
                answer = yield this._callback(request.data, request, response);
            }
            catch (err) {
            ...
            try {
                if (response.statusCode === response.OK) {
                    ...
                    const queueMessage = new QueueMessage_js_1.default('ok', answer);
                    ...
                    const queueMessage = new QueueMessage_js_1.default('error', response.statusMessage);
                    reply = queueMessage.serialize();
                }
            }
            catch (err) {
    ...
    _sendStatus(channel, replyTo, correlationId, status, message = '') {
        const reply = new QueueMessage_js_1.default(status, message);
    ...
    unserializeMessage(channel, msg) {
        ...
        let request;
        try {
            request = QueueMessage_js_1.default.unserialize(msg.content);
...
exports.default = GatheringServer;
